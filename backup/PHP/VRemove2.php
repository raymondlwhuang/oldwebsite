...
include("sethash.php");
if($_SESSION["admin"]==1 && isset($_SESSION["FriendID"]) && $_SESSION["FriendID"]!=$GV_id)
{
...
if ($_SESSION["admin"]==1 && isset($_SESSION["FriendID"]) && $_SESSION["FriendID"]!=$GV_id) echo "$FriendName's video<input type=\"checkbox\" value=\"\" id=\"other_video\" onchange=\"other_video();\">";
if(isset($video)){
	foreach ($video as $key1 => $value1) {
...
function other_video() {
	if(document.getElementById('other_video')) {
		if(document.getElementById('other_video').checked) var FriendID= <?php echo $FriendID; ?>;
