...
if ($_FILES){
	...
	echo "
        <script type=\"text/javascript\">
			window.open('PictureUploader.php',target='_top');
        </script>";
	exit();		
}
...
<table>
	...
	<tr>
		<td><?php if($bname =="Internet Explorer" || $bname =="Apple Safari") echo "Picture Upload(Max 2MB):"; else echo "Mulitiple Pictures Upload(Max 2MB each):"?></td>
		<td align="left">
		<input id="infile" name="infile[]" type="file" accept="image/*" onChange="document.getElementById('uploader').submit();" <?php if($bname =="Internet Explorer" || $bname =="Apple Safari") echo ""; else echo 'multiple="true"' ?> size="40" style="font-size:20px;border-color:#5050FF;border-width: 3px;"/> 
		</td>
	</tr>
</table>
</form>
