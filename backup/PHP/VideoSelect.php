...
//	$group_list[$GV_owner_path] = $viewer_group;
	$queryVideo="SELECT * FROM picture_video where  owner_path = '$GV_owner_path' and picture_video = 'videos' and (viewer_group = '$viewer_group' or viewer_group = '')  order by name";
...
//	echo "<br/>";
		if(!isset($group_list[$GV_owner_path])) $group_list[$GV_owner_path] = $viewer_group;
		$video_list[$GV_owner_path][$viewer_group][] = $row3['name'];
		$disp_name[$GV_owner_path][$viewer_group][] = substr($row3['name'],strripos($row3['name'],"/") + 1);
	} 
}
