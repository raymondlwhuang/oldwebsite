...
if(isset($_REQUEST['text_to_display'])) {
	$text = new textjpeg;
	if (isset($_REQUEST['filename'])) $text->filename = mysql_real_escape_string($_REQUEST['filename']); // text to display
	if (isset($_REQUEST['text_to_display'])) $text->text_to_display = mysql_real_escape_string($_REQUEST['text_to_display']); // text to display
	if (isset($_REQUEST['font'])) $text->font = mysql_real_escape_string($_REQUEST['font']); // font to use (include directory if needed).
	...
	if(isset($_REQUEST['toNewPic'])) {
		$text->toNewPic = mysql_real_escape_string($_REQUEST['toNewPic']);
//		unlink($toNewPic);
	}
	else $text->toNewPic = "";
	$text->draw(); // GO!!!!!
