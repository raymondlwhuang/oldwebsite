...
if (isset($_POST['Submit']))
{
	 ...
	 )
	 {
		...
		if (mysql_num_rows($result2) > 0){
				$ErrorMessage = "**Duplication User Exist(email_address: $email_address)**";
		}
		ELSE {
...
else if (isset($_GET['ResetUser']))
{
	...
	$email_address = newdecode($_GET['ResetUser']);
	mysql_query("DELETE FROM user WHERE email_address = '$email_address'");	
	mysql_close();
//echo newdecode($_GET['ResetUser']);
//exit();
}
else {
