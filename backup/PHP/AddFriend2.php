...
if($action==0){
	...
	if(mysql_num_rows($QueryResult) == 0) {
		if($viewer_email !=$owner_email) mysql_query("INSERT INTO view_permission(user_id,owner_email,owner_path,viewer_email,name,is_active) VALUES($user_id,'$owner_email','$owner_path','$viewer_email','$name',$friendnow)");
	}
	else {
	...
	$FriendResult = mysql_query("SELECT * FROM view_permission WHERE viewer_email = '$owner_email'");
		...
		while($row = mysql_fetch_array($Friends)) {
			...
			while($Picture = mysql_fetch_array($PictureResult)) {
				...
				if(mysql_num_rows($QueryResult1) == 0) { // list if not found
					if($friendnow==-1)	mysql_query("INSERT INTO view_permission(user_id,owner_email,owner_path,viewer_email,name,is_active) VALUES($user_id,'$owner_email','$owner_path','$cur_email','$name',$friendnow)");
...
if(isset($optionEmail)) {
	...
	echo "
		...
		<input type=\"image\" src=\"$optionPicture[$id]\" width='50px'>
		</td>";
	echo "	
