...
_END;
			...
			?>
			<input type="text" size="45" maxlength="400" name="SearchGroup" id="searchField" onKeyUp="stopCount();SendRequest('../search/MyMsg.php','ChatMsg');" name="s_message" style="border-color:#0000ff;border-style:ridge;" />
			...
			if(isset($profile_picture)){
				foreach ($profile_picture as $key => $value) {
					...
					if($FriendStat[$key] == 3 || $FriendStat[$key] == 4)
					{
						...
						<input type='checkbox' name='TalkTo' id ='$FriendEmail[$key]' value='$TalkValue[$key]' $TalkStatus[$key] onClick="SendRequest ('ChatTo.php?email=$FriendEmail[$key]','BlankMsg');"/>
END;
					}
