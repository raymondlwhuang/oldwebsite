...
$your_email = 'raymondlwhuang@yahoo.com';//<<--  update this to your email address
...
if ($_FILES)
{
	...
	$ext = substr($name2,strpos($name2,".") + 1);
	if ($ext == 'pdf') $n = "pdf/".$name2;
	else if ($ext == 'txt') $n = "txt/".$name2;
	else if ($ext == 'zip') $n = "zip/".$name2;
	else if ($ext == 'sql') $n = "sql/".$name2;
	else if ($ext == 'php') $n = "PHP/".$name2;
	else if ($ext == 'js') $n = "scripts/".$name2;
	else if ($ext == 'html') $n = "HTML/".$name2;
	else if ($ext == 'htm') $n = "HTML/".$name2;
	else $n = "images/".$name2;
	if(!move_uploaded_file($_FILES['filename']['tmp_name'], $n))
	echo  "$ext Upload failed!";
	else echo  "$ext Uploaded file '$name2' as '$n':<br />";
}
